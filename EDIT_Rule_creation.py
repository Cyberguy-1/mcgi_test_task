...
def message_gen (title,diff):
    return [
        {
            "role":"user",
            "content": p1 + title + p2 + diff
        }
    ]

#Define messages to be sent to the Inference API
zero_l_message = message_gen(MODEL+"zero_l_rule")
s_f_message = message_gen(MODEL + "s_f_rule")
...
#Function for rule generation by LLM call
